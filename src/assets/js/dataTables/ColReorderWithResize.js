/**
 * Created by Paulo Martins paulo.martins@rwinteractive.com on 15/05/2017.
 */
 ...
 *  5. When the table has table-layout: fixed, we need to set a minWidth, otherwise we can resize the column to zero width.
 ...
 * Modified:    May 16th 2017 by Paulo Martins - pabm71@gmail.com
 * Language:    Javascript
    ...
    ColReorder.prototype = {
        ...
        "_fnMouseDown": function ( e, nTh, i )
        {
            ...
            if ($(nTh).css('cursor') == 'col-resize') {
                ...
                // console.log('this.s.mouse.startX', this.s.mouse.startX);
                // console.log('this.s.mouse.startWidth', this.s.mouse.startWidth);
                // console.log('this.s.mouse.resizeElem', this.s.mouse.resizeElem);
                ////////////////////
                //Martin Marchetta
        ...
        "_fnMouseMove": function ( e, colResized )
        {
            ...
            if (this.dom.resize) {
                ...
                // Block the resize if width <= this.s.minWidth and the movement is to decrease the width
                // Except when the column has the attr column-min-width
                // When this happens the min width is controlled by the value in that attr
                var columnMinWidth = $(nTh).attr('column-min-width');
                if (typeof columnMinWidth !== typeof undefined && columnMinWidth !== false){
                    if ($(nTh).width() <= columnMinWidth && moveLength < 0) {
                        $(nTh).width(columnMinWidth);
                    }
                }else{
                    if ($(nTh).width() <= this.s.minWidth && moveLength < 0) {
                        $(nTh).width(this.s.minWidth);
                    }
                }
                ...
                // Create an event for when the resize begin
                var customEvent = new CustomEvent('resizeBegin');
                window.dispatchEvent(customEvent);

                // console.log('resizing');
                // console.log('moveLenght', moveLength);
                // console.log('$(nTh).width()', $(nTh).width());
                // console.log('this.s.minWith', this.s.minWidth);
                // console.log('$(nTh)', $(nTh));
                ...
                if($('div.dataTables_scrollBody') != null){
                    ...
                    if($('div.dataTables_scrollBody').length > 0){
                        ...
                        // console.log('tableScroller', tableScroller);
                        // console.log('tableScroller', $(tableScroller));
                        ...
                        // console.log('scrollingTableHead', scrollingTableHead);
                        // console.log('scrollingTableHead', $(scrollingTableHead));
                        ...
                        $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width(this.s.mouse.startWidth + moveLength);

                        console.log('resize the columns width', $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width());

                        // we need to limit the column width minimum size
                        if (typeof columnMinWidth !== typeof undefined && columnMinWidth !== false){
                            if ($($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width() <= columnMinWidth && moveLength < 0) {
                                $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width(columnMinWidth);
                            }
                        }else{
                            console.log('check if limit is needed', this.s.minWidth);
                            console.log('move lenght', moveLength);
                            console.log('column width', $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width());
                            if ($($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width() <= this.s.minWidth && moveLength < 0) {
                                console.log('limit is needed');
                                $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width(this.s.minWidth);
                            }
                        }

                        var visibleHeader = $(nTh).width();
                        var hiddenHeader = $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width();

                        // get the elements width
                        console.log('header th width', visibleHeader);
                        console.log('hidden header th width', hiddenHeader);

                        if (visibleHeader != hiddenHeader){
                            console.log('set invisible header equal to visible header');
                            $($(scrollingTableHead)[0].childNodes[1].childNodes[visibleColumnIndex]).width(visibleHeader);
                        }
                        ...
                        if(scrollXEnabled){
                            console.log('resize the table');
                            $('.dataTables_scrollHeadInner').width(this.table_size + moveLength);
                ...
                console.log('resize the columns no scroll body');
